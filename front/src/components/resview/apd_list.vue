    ...
    <div class="cardview">
        <div class="cardview_body">
            <div style="padding-top:30px; padding-bottom:16px; width: 100%;">
                <h5>这里展示所有（审核通过的）预约申请生成的通过记录，只有审核通过的申请会有对应的此类记录</h5>
                <hr>
                <div>
                ...
                <div class="row" v-if="total">
                    ...
                    <div class="col-lg-8 text-end align-middle" style="padding-right: 30px;">
                        ...
                        <div class="form-check form-check-inline">
                            ...
                            <label class="form-check-label" for="inlineRadio2">审核时间</label>
                        </div>
                    ...
                    <tbody id="vlist" class="all_tb">
                        <tr v-for="(item, index) in items" :key="item.id">
                            ...
                            <td class="tbr align-middle">
                                <img title="用户" :src="`http://127.0.0.1:8000${item.origin_resvation.res_user.profile}`"
                                    alt="avatar" width="26" height="26" class="rounded-circle me-2" />
                                {{ item.origin_resvation.res_user?.username || '无' }}
                            </td>
                            <td class="tbr align-middle">{{ item.id }}</td>
                            <td class="tbr align-middle">{{ item.origin_resvation.res_start_time.substr(0, 10) }}</td>
                            <td class="tbr align-middle">{{ item.origin_resvation.device.name }}</td>
                            <td class="tbr align-middle">
                                <template v-if="item.state_apd === '未生效'">
                                    <span class="badge bg-primary">未生效</span>
                                </template>
                                <template v-else-if="item.state_apd === '已认证'">
                                    <span class="badge bg-success">已认证</span>
                                </template>
                                <template v-else-if="item.state_apd === '待认证'">
                                    <span class="badge bg-danger">待认证</span>
                                </template>
                                <template v-else>
                                    <span class="badge bg-secondary">已过期</span>
                                </template>
                            </td>
                            <td class="tbr align-middle">{{ item.origin_resvation.audit_staff?.username || '无' }}</td>
                            <td class="tbr align-middle">{{ item.origin_resvation.inviter?.username || '无' }}</td>
                            <td class="tbr align-middle">{{ item?.completed_time || "未通过" }}</td>
                            <td>
                ...
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="staticBackdropLabel">生效记录</h5>
    ...
    methods: {
        ...
        async set_dialog_item(i){
            try {
                ...
                const origin_resvation = data.origin_resvation;
                this.v_id = data.id;
                this.v_user = origin_resvation.res_user.username;
                this.v_res_time = origin_resvation.res_time;
                this.v_pr_time = origin_resvation.audit_time;
                this.v_pr_user = origin_resvation.audit_staff?.username || '无'
                this.v_tag = data.state_apd;
                if (this.v_tag == "未生效"){
                    this.type = "primary";
                } else if (this.v_tag == "已认证"){
                    this.type = "success";
                } else if (this.v_tag == "待认证") {
                    this.type = "warning";
                }else{
                    this.type = "danger";
                }
                ...
                this.v_cmp_time = data.completed_time || "未通过";
        ...
        async get_res_list(page_num = 1) {
            ...
            try {
                const response = await axios.post("http://127.0.0.1:8000/api/v_apd_report", params);
